...
| Section 2. 인증 관통 기능 | 부분 완료 | 백엔드 완료, 프론트 관통 흐름 미완료 |
...
현재 개발 위치는 **Section 4 백엔드 핵심 완료 후, Section 2~4에서 미뤄 둔 프론트 관통 기능을 시작하기 직전**이다.
...
- [ ] 프론트 회원가입·로그인 화면
- [ ] 프론트 API 모듈
- [ ] 로그인 → JWT 저장 → 내 정보 조회 실제 연동
- [ ] 프론트를 포함한 인증·보안 관통 테스트
...
- [ ] 브라우저에서 회원가입부터 내 정보 표시까지 실제 HTTP로 동작
- [ ] `npm run lint` 성공
- [ ] `npm run build` 성공
